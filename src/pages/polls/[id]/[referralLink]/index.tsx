...
const PollReferralLinksPage = () => {
  const { id: pollId, referralLink } = useParams<{
    id: string;
    referralLink: string;
  }>();
  ...
  // ---- Build params (kind=poll, entityId=pollId) ----
  const params = useMemo(() => {
    ...
    return p;
  }, [page, sortByViews, sortDir, pollId]);
  ...
  const { data, isLoading, error, refetch } = useApiQuery(urlWithQuery, {
    key: ["referral-listing-poll", pollId, urlWithQuery],
  } as any);
    ...
    <div className="flex flex-col gap-6 md:px-4 h-full">
      ...
      <section className="flex flex-col gap-4 max-w-6xl mx-auto w-full">
        ...
        {!isLoading && !error && !items.length && (
          <p className="text-sm text-muted-foreground">
            No referral links found for this poll.
          </p>
        )}
      </section>
        ...
        {items.map((row) => {
          ...
          return (
            <Card
              key={row._id}
              onClick={() =>
                navigate(`/polls/${pollId}/${referralLink}/${row._id}`)
              }
              className="rounded-3xl bg-primary/5 hover:bg-primary/10 flex flex-col md:flex-row md:items-stretch gap-4 p-4 cursor-pointer"
            >
              {/* Left: basic info */}
              <div className="flex items-start gap-3 md:w-1/3">
                ...
                <div className="flex flex-col gap-1 text-xs md:text-sm">
                  ...
                  <span className="text-[11px] text-muted-foreground">
                    Sharer ID: {row.sharerExternalAccountId}
                  </span>
                  {cityName && (
                    <span className="text-[11px] text-muted-foreground">
                      {cityName}
...
export default PollReferralLinksPage;
