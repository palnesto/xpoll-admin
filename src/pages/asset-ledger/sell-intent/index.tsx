...
export default function SellIntent() {
  ...
  const [selectedNetwork, setSelectedNetwork] = useState<WalletName | null>(
    null
  );
  ...
  const isValidXrplClassic = (addr: string) =>
    /^r[1-9A-HJ-NP-Za-km-z]{25,35}$/.test(addr);
  const isValidAptosAddress = (addr: string) =>
    /^0x[0-9a-fA-F]{1,64}$/.test(addr);
  ...
  const ensureSuiFunds = useCallback(
    async (requiredMist: bigint) => {
      ...
      if (total < requiredMist) {
        ...
        throw new Error(
          `Insufficient SUI balance (have ${have.toFixed(
            3
          )} SUI, need ${need.toFixed(3)} SUI including fees)`
        );
      }
    },
  ...
  const ensureExpectedWallet = useCallback(
    (actual: string | null | undefined, expected: string, label: string) => {
      ...
      if (actual.toLowerCase() !== expected.toLowerCase()) {
        throw new Error(
          `${label} wallet does not match the authorized admin address.`
        );
      }
    },
    []
  ...
  const applyRemarks = useCallback(
    (entries: Array<{ _id: string; remark?: string }>) => {
      if (!entries.length) return;
      const remarkMap = new Map(
        entries.map((entry) => [entry._id, entry.remark])
      );
    },
    []
  );
  ...
  const persistAptosResults = useCallback(
    async (
      ...
      try {
        ...
        appToast.error(
          err?.message || "Failed to record Aptos batch results on backend"
        );
      }
    },
    [adminAptos]
  ...
  const validateAddresses = useCallback(
    async (
      network: Extract<WalletName, "XRP" | "APTOS">,
      items: WithdrawReq[]
    ) => {
      if (!items.length) {
        return [] as Array<{
      ...
      try {
        const { data } = await api.post(
          ...
          {
            network,
            addresses: items.map(({ _id, walletAddress }) => ({
              id: String(_id),
              walletAddress,
            })),
          },
      ...
      if (batch) {
        try {
          ...
          if (Array.isArray(decoded)) {
            ...
            if (successCount) {
              appToast.success(
                `Transferred ${successCount} Aptos request${
                  successCount > 1 ? "s" : ""
                }.`
              );
            }
            if (failureCount) {
              appToast.error(
                `Failed to transfer ${failureCount} Aptos request${
                  failureCount > 1 ? "s" : ""
                }${failureRefs.length ? ` (${failureRefs.join(", ")})` : ""}.`
              );
            }
          }
        } catch (err) {
  ...
  const transferSuiBatch = useCallback(
    async (items: WithdrawReq[]) => {
      if (!currentSui?.address) throw new Error("Connect Sui wallet first");
      ...
      const tx = new Transaction();
      tx.setSender(currentSui.address);
      const totalMist = items.reduce(
        (sum, req) => sum + toMist(Number(req.parentAmountVal)),
        0n
      );
  ...
  const transferXrpViaXaman = useCallback(
    async (req: WithdrawReq) => {
      if (!adminXrp) throw new Error("Connect XRP (Xaman) wallet first");
      ensureExpectedWallet(adminXrp, adminXrpAddress, "XRP");
      if (!isValidXrplClassic(req.walletAddress))
        throw new Error("Invalid XRPL address for destination");
  ...
  const submitXrpBatch = useCallback(async (items: WithdrawReq[]) => {
    ...
    try {
      const { data } = await api.post(
        ...
        {
          network: "XRP",
          transfers: items.map(({ _id, walletAddress, parentAmountVal }) => ({
            id: String(_id),
            walletAddress,
            amount: Number(parentAmountVal),
          })),
        },
        { signal: controller.signal }
      );
      ...
      if (err?.name === "AbortError") {
        throw new Error(
          "Batch transfer request timed out. Please retry or check backend connectivity."
        );
      }
      throw err;
    } finally {
  ...
  const { mutateAsync: approveSellIntentMutation } = useApiMutation<
    { actionIds: string[] },
    { approvedIds: string[] }
  >({
    ...
    onSuccess: (data: { approvedIds: string[] }) => {
      ...
      if (ids.length > 0) {
        appToast.success(
          `Successfully approved ${ids.length} sell intent${
            ids.length > 1 ? "s" : ""
          }`
        );
      }
  ...
  const bulkApproveSellIntents = useCallback(
    async (actionItems: Array<{ actionId: string; txnHash: string }>) => {
  ...
  const batchTransfer = useCallback(async () => {
    ...
    if (selectedNetwork === "APTOS") {
      ...
      setIsBatchProcessing(true);
      const invalid = pendingFiltered.filter(
        (req) => !isValidAptosAddress(req.walletAddress)
      );
      if (invalid.length) {
        appToast.error(
          `Invalid Aptos address${invalid.length > 1 ? "es" : ""}: ${invalid
            .map((r) => r.id)
            .join(", ")}`
        );
        return;
        ...
        applyRemarks(
          activationResults.map((item) => ({
            ...
            remark: item.active
              ? undefined
              : item.reason ||
                "Wallet address is not active on ledger. Please activate it on ledger and try again.",
          }))
        );
      } catch (err: any) {
        console.error("Aptos address activation check failed", err);
        appToast.error(
          err?.message || "Failed to verify Aptos wallet activation."
        );
        return;
      }
      ...
      if (inactiveAptos.length) {
        appToast.error(
          `Cannot start Aptos batch. Inactive address${
            inactiveAptos.length > 1 ? "es" : ""
          }: ${inactiveAptos
      ...
      try {
        ...
        for (let i = 0; i < batches.length; i++) {
          ...
          appToast.info(
            `Processing APTOS batch ${i + 1}/${batches.length} (${
              batch.length
            } transfers)…`
          );
        ...
        if (globalSuccess.size) {
          appToast.success(
            `Transferred ${globalSuccess.size} Aptos request${
              globalSuccess.size > 1 ? "s" : ""
            } across ${batches.length} batch${batches.length > 1 ? "es" : ""}.`
          );
        }
        if (globalFailures.length) {
          appToast.error(
            `Failed to transfer ${globalFailures.length} Aptos request${
              globalFailures.length > 1 ? "s" : ""
            }: ${globalFailures
    ...
    if (selectedNetwork === "SUI") {
      ...
      try {
        for (let i = 0; i < batches.length; i++) {
          const batch = batches[i];
          appToast.info(
            `Processing SUI batch ${i + 1}/${batches.length} (${
              batch.length
            } transfers)…`
          );
          ...
          appToast.success(
            `Completed SUI batch ${i + 1}/${batches.length}${
        ...
        if (totalSuccess) {
          appToast.success(
            `Transferred ${totalSuccess} SUI request${
              totalSuccess > 1 ? "s" : ""
            } across ${batches.length} batch${batches.length > 1 ? "es" : ""}.`
          );
        }
    ...
    if (selectedNetwork === "XRP") {
      ...
      try {
        ...
        appToast.error(
          err?.message || "Failed to verify XRP wallet activation."
        );
        return;
      }
      ...
      if (inactiveXrp.length) {
        appToast.error(
          `Cannot start XRP batch. Inactive address${
            inactiveXrp.length > 1 ? "es" : ""
          }: ${inactiveXrp
            .map((item) => item.walletAddress)
            .join(", ")}. Please activate them on ledger and try again.`
        );
      ...
      try {
        ...
        for (let i = 0; i < batches.length; i++) {
          const batch = batches[i];
          appToast.info(
            `Processing XRP batch ${i + 1}/${batches.length} (${
              batch.length
            } transfers)…`
          );
    ...
    if (successes.length) {
      appToast.success(
        `Transferred ${successes.length} ${selectedNetwork} request${
    ...
    if (failures.length) {
      appToast.error(
        `Failed to transfer ${failures.length} ${selectedNetwork} request${
          failures.length > 1 ? "s" : ""
        }: ${failures.join(", ")}`
      );
    }
    ...
    <div className="space-y-6">
                ...
                filteredRequests.map((r) => (
                  <tr key={r.id} className="border-t border-gray-100">
                    ...
                    <td className="px-4 py-2">
                      ...
                      {r.metadata.status === "TRANSFERRED" && (
                        <span className="text-green-600">Transferred</span>
                      )}
                      {r.metadata.status === "REJECTED" && (
      ...
      {xamanQRModalOpen && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="xaman-payment-qr-title"
        >
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xl font-semibold">
                  {selectedNetwork ?? "Select a network"} payout
                </p>
              </div>
            </div>
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-base">
                ...
                <p className="text-white truncate font-semibold">
                  {selectedNetwork ? `${selectedNetwork} wallet` : "—"}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-amber-200 bg-amber-50 px-5 py-4 text-base text-amber-900">
            ...
            <p className="mt-1 text-amber-800/80">
              Ensure sufficient balance is available in the connected admin
              wallet and double-check all wallet addresses. This action will
              initiate on-chain transfers for every pending request.
            </p>
          </div>
          ...
          <div className="rounded-xl border border-black/10 bg-white px-5 py-4 text-sm text-black/70">
            ...
            <ul className="mt-2 space-y-1.5">
              <li>
                • Each request is processed sequentially with the configured
                delays.
              </li>
              <li>• Successes are marked automatically on completion.</li>
              <li>
                • Any failures will surface in the activity feed for review.
              </li>
            </ul>
          </div>
        </div>
