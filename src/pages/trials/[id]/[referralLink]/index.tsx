// src/pages/trials/[id]/[referralLink].tsx
...
const TrialReferralLinksPage = () => {
  const { id: trialId, referralLink } = useParams<{
    id: string;
    referralLink: string;
  }>();
  ...
  // ---- Build params: kind=trial, entityId = trialId ----
  const params = useMemo(() => {
    ...
    return p;
  }, [page, sortByViews, sortDir, trialId]);
  ...
  const { data, isLoading, error, refetch } = useApiQuery(urlWithQuery, {
    key: ["referral-listing-trial", trialId, urlWithQuery],
  } as any);
    ...
    <div className="flex flex-col gap-6 md:px-4 h-full">
      ...
      <section className="flex flex-col gap-4 max-w-6xl mx-auto w-full">
        ...
        {!isLoading && !error && !items.length && (
          <p className="text-sm text-muted-foreground">
            No referral links found for this trial.
          </p>
        )}
      </section>
        ...
        {items.map((row) => {
          ...
          return (
            <Card
              key={row._id}
              onClick={() =>
                navigate(`/trials/${trialId}/${referralLink}/${row._id}`)
              }
...
export default TrialReferralLinksPage;
