// src/pages/polls/[id]/[referralLink]/[uniqueLinks].tsx
...
type UniqueRow = {
  _id: string;
  linkId: string;
  viewerExternalAccountId: string;
  firstAt: string;
  lastAt: string;
  visits: number;
  username: string;
...
const PollReferralUniqueLinksPage = () => {
  const { id: pollId, uniqueLinks: linkId } = useParams<{
    id: string;
    referralLink: string;
    uniqueLinks: string;
  }>();
  ...
  // ---- Guard: linkId is required ----
  const effectiveLinkId = linkId || "";
  ...
  const params = useMemo(() => {
    const p: Record<string, any> = {
      page,
      pageSize: PAGE_SIZE,
      referralLinkIds: effectiveLinkId,
    };
    ...
    return p;
  }, [page, sortByVisits, sortDir, effectiveLinkId]);
  ...
  const urlWithQuery = useMemo(() => {
    ...
    const base = endpoints.referral.uniques;
    const qs = usp.toString();
    return qs ? `${base}?${qs}` : base;
  }, [params]);

  const { data, isLoading, error, refetch } = useApiQuery(urlWithQuery, {
    key: ["referral-uniques", effectiveLinkId, urlWithQuery],
  } as any);
  ...
  const payload: UniqueListingResponse | undefined = data?.data?.data;
  const items: UniqueRow[] = payload?.items ?? [];
  ...
  if (!effectiveLinkId) {
    return (
      <div className="p-4">
        ...
        <Button onClick={() => navigate(`/polls/${pollId}/referrals`)}>
      ...
      <section className="flex flex-col gap-4 max-w-6xl mx-auto w-full">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <Button
              ...
              onClick={() => navigate(`/polls/${pollId}/referrals`)}
            >
            ...
            <div className="flex flex-col gap-1">
              ...
              <p className="text-[11px] md:text-xs text-muted-foreground break-all">
                Referral Link ID: {effectiveLinkId}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 text-sm">
          <div className="flex items-center gap-2">
            ...
            <Switch
              checked={sortByVisits}
              onCheckedChange={(v) => {
                setSortByVisits(v);
                setPage(1);
              }}
              aria-label="Toggle sort by visits"
            />
          </div>
        ...
        {error && !isLoading && (
          <p className="text-sm text-red-500">
            Failed to load unique visitors.
          </p>
        )}
        {!isLoading && !error && !items.length && (
          <p className="text-sm text-muted-foreground">
            No unique visitors found for this referral link.
          </p>
        )}
      </section>
              ...
              <div className="flex items-start gap-3 md:w-1/3">
                ...
                <div className="flex flex-col gap-1 text-xs md:text-sm">
                  ...
                  <span className="text-[11px] text-muted-foreground">
                    Viewer ID: {row.viewerExternalAccountId}
                  </span>
                  {cityName && (
                    <span className="text-[11px] text-muted-foreground">
                      {cityName}
                  ...
                  {typeof row.civicScore === "number" && (
                    <span className="text-[11px] text-muted-foreground">
                      Civic Score: {row.civicScore}
                    </span>
                  )}
                </div>
              </div>

              <CardContent className="flex-1 flex flex-col md:flex-row gap-4 justify-between text-xs md:text-sm p-0">
                <div className="flex flex-col gap-1">
                  <span className="font-medium text-[11px] text-muted-foreground">
                    Visits
                  </span>
                  <span className="px-2 py-0.5 rounded-full border bg-background text-[11px] w-fit">
                ...
                <div className="flex flex-col gap-1 md:text-right">
                  ...
                  <span>
                    First visit:{" "}
                    {row.firstAt ? utcToAdminFormatted(row.firstAt) : "—"}
                  </span>
                  <span>
                    Last visit:{" "}
                    {row.lastAt ? utcToAdminFormatted(row.lastAt) : "—"}
                  </span>
                </div>
              </CardContent>
            </Card>
...
export default PollReferralUniqueLinksPage;
