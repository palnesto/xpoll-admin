...
import { createXamanPayload, getXamanPayload, waitForXamanAccount } from '@/lib/xaman';
...
export default function XamanConnect({ className = '', onConnected, onDisconnected }: XamanConnectProps) {
      ...
      xummSocketRef.current.onmessage = async (evt) => {
        try {
          ...
          if (msg?.expired) {
            ...
            setXrp({ connecting: false, connected: false, error: 'Xaman request expired' });
          ...
          if (msg?.signed === false) {
            ...
            setXrp({ connecting: false, connected: false, error: 'User rejected in Xaman' });
            setXamanQRModalOpen(false);
            xummSocketRef?.current?.close();
            return;
          }
          ...
          setXrp({ connecting: false, connected: false, error: err?.message || 'Xaman connect failed' });
          xummSocketRef?.current?.close();
        }
      };

      xummSocketRef.current.onerror = () => toast.error('Xaman websocket error');
    } catch (e: any) {
      console.error('Xaman connect error:', e);
      toast.error(e?.message || 'Xaman connect failed');
      setXrp({ connecting: false, connected: false, error: e?.message || 'Xaman connect failed' });
    }
  }, [isMobile]);
  ...
  return (
    <>
      ...
      {/* Xaman Modal */}
      {xamanQRModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" role="dialog" aria-modal="true" aria-labelledby="xaman-qr-title">
          <section
          ...
          >
            ...
            <div className="mb-4 text-center">
              <h4 id="xaman-qr-title" className="text-xl md:text-2xl font-semibold tracking-tight">
                Scan with Xaman
              </h4>
              <p className="text-sm md:text-[13px] text-white/70 mt-1">
                Open the <span className="font-medium">Xaman</span> app on your phone and scan the QR to connect.
              </p>
            </div>
            ...
            <div className="mx-auto rounded-2xl p-3 md:p-4 bg-white shadow-lg border border-black/5 max-w-xs">
              {xamanQR ? (
                <img src={xamanQR} alt="Xaman sign-in QR" className="mx-auto h-60 w-60 object-contain" />
              ) : (
            ...
            <div className="mt-4 text-center text-xs text-white/60">
              Having trouble? Ensure your phone and computer are online, then refresh and try again.
            </div>
          </section>
        </div>
      )}
